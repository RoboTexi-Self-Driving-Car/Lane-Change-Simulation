 ...
 public:
  ...
  node(T *points, int size, int depth = 0) {
    ...
    if (is_even) {
      ...
      std::sort(points, points + size,
                [](T const &a, T const &b) { return a.y < b.y; });
    }
    ...
    if (size - median - 1 > 0) {
      this->right =
          new node<T>(points + median + 1, size - median - 1, depth + 1);
    }
  }
   ...
   *
   *  @return Return true if the node is the leaf, and return false if the node
   * isn't the leaf.
   */
  inline bool is_leaf() {
  ...
  node<T> *nearest(node<T> *query, const int depth = 0) {
    ...
    // Calculate distance between the query and self along one direction (if
    // depth is even, the direction is horizontal)
    bool is_even = !(depth & 1);
  ...
  void _radius_nearest(std::vector<node<T> *> &neighbors, node<T> *query,
                       const double r, const int depth = 0) {
    // Calculate distance between the query and self along one direction (if
    // depth is even, the direction is horizontal)
    bool is_even = !(depth & 1);
    double dx = query->point.x - this->point.x;
    ...
    if (query->distance(this) <= r) {
      if (neighbors.size() == 0) {
        ...
        for (typename std::vector<node<T> *>::iterator it = neighbors.begin();
             it <= neighbors.end(); ++it) {
          if (it == neighbors.end()) {
            neighbors.push_back(this);
            break;
  ...
  void _k_nearest(std::vector<node<T> *> &neighbors, node<T> *query,
                  const int k, const int depth = 0) {
    double max_distance;
    if (neighbors.size() > 0) {
    ...
    if (left) {
      ...
      if (this->has_right_node() &&
          (neighbors.size() < k || distance < max_distance)) {
...
class kdtree {
 public:
   ...
   *  @return The nearest neighbor node.
   */
  node<T> *nearest(T query_point) { return this->root->nearest(query_point); }
   ...
   *  @return The nearest neighbor node.
   */
  node<T> *nearest(node<T> *query) { return this->root->nearest(query); }
