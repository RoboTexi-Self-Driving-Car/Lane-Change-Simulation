...
const vector<std::string> DecisionAgent2::hostActions = {"normal", "left",
                                                         "right"};
const vector<std::string> DecisionAgent2::otherActions = {"acc", "dec",
                                                          "normal", "stop"};
...
void DecisionAgent2::ApplyAction(const Model& model, int agentIndex,
...
float DecisionAgent2::evaluationPath(const Model& mo, const vector<vec2f>& path,
                                     vector<int>& carintentions) {
  Model model(mo);
  float score = 0.0;
...
bool DecisionAgent2::getPath(const Model& model, vector<vec2f>& mypath,
                             vector<int>& carintentions) {
...
bool DecisionAgent2::getPath2(const Model& model, vector<vec2f>& mypath,
                              vector<int>& carintentions) {
  // std::string bestAction = "stop";
  // int numAgents = model.getCars().size();
  vector<string> legalactions = generateLegalActions(model);
...
bool DecisionAgent2::isCloseToOtherCar(Car* mycar, const Model& model) const {
  ...
  if (abs(obstaclecar->getPos()[0] - mycar->getPos()[0]) <
          Globals::constant.BELIEF_TILE_SIZE * 1.2 &&
