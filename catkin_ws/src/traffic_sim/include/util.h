...
/**
 * Observe the model from the view of the host car.
 *
 */
void observe(Host* car, const Model& model) {
  ...
  for (int i = 0; i < cars.size(); i++) {
    Agent* car = dynamic_cast<Agent*>(cars[i]);
    int car_index = model.toindex(car);
    Inference::MarginalInference* inference = car->getInference(car_index+1, model);
...
/**
 * Intention Detector
 *   - host state using the pointer to host
 *   - observe(car, model)
 *   - get belief for each car, and change color
 *   - collect car intentions  -->  car_intentions
 *
 */
vector<int> infer(const Model& model) {
  ...
  for (int k = 0; k < cars.size(); k++) {
    ...
    vector<float> belief = car->getInference(index+1, model)->getBelief(); // [cooperative, aggressive]
    // if (k != index)
    //   cout<<"I am here"<<endl;
    int maxindex = 0;
    ...
    car_intentions.push_back(maxindex);
    display.colorchange(car, colors[maxindex]);
  }
  return car_intentions;
}
